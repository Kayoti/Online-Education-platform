...
$students = $fgmembersite->GetProfessorStudents($course_id);                  
 }                 
?>
<!DOCTYPE html>
...
<section class="content">
	...
	<div class="container">
		<div class="row">
			<div class="span8">
				<p>Please approve students as they’re added.</p>
				<table class="table table-bordered">
					<thead>
						<tr>
							<th style="vertical-align: bottom; text-align: center;">#</th>
							<th>Name</th>
							<th>Status</th>
							<th style="text-align: center">Audio Only submission</th>
						</tr>
					</thead>
					<tbody>
                         <?php 
                    ...
                    foreach ($students as $student){
                     ...
                     $i++;
                     echo "<tr><td style='vertical-align: bottom; text-align: center;'>$i</td><td>";
                     echo "<a href='evaluation.php?student_id=$s_id&course_id=$course_id'>";
                     echo $student["s_fname"]."  ".$student["s_lname"];
                     ...
                     if($student["approval_status"] == 0){
						...
						echo "</td><td style='vertical-align: bottom; text-align: center;'>";
						echo "<form id='audioform' action='audio.php?course_id="; echo $course_id."'"; echo" method='post' accept-charset='UTF-8'>";
                    echo "<input type='hidden' name='submitted' id='submitted' value='1'/>";
						 if($student["audio_only"] == 0)
						 {
						 echo " <input type='checkbox' name='a_student[]' class= 'chk_boxes' value=$s_id>
						 <input class='btn btn-small btn-logo ' type='button' name='submit' value='Off'/>";
						 }
						 if($student["audio_only"] == 1)
						 {
							echo "<input type='checkbox' name='a_student[]' class= 'chk_boxes' value=$s_id>
						 <input class='btn btn-small btn-success ' type='submit' name='submit' value='On'/>";
						 }
                     echo "</td></tr>";
                  }?>  
			...
			<div class="span4">
                <?php 
                    if($pending_approval){
                        echo "<a class='btn btn-large btn-logo span2 offset1' href='approvestudents.php?course_id=$course_id'>Approve Students</a>";
                    }
                    else{
                        echo "<a class='btn btn-large span2 offset1 disabled' href='#'>Approve Students</a>";
                    }
                ?>

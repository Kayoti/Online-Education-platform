...
$all_topics = array();
if(($topics["topic_name1"]!='')&&($topics["topic_name1"]!=NULL))
{
	$all_topics[] = $topics["topic_name1"];
}
if(($topics["topic_name2"]!='')&&($topics["topic_name2"]!=NULL))
{
	$all_topics[] = $topics["topic_name2"];
}
if(($topics["topic_name3"]!='')&&($topics["topic_name3"]!=NULL))
{
	$all_topics[] = $topics["topic_name3"];
}
$tot_topics = sizeof($all_topics);
$topic_no = mt_rand(0,$tot_topics-1);
$topic_selected = $all_topics[$topic_no];


	if(!$fgmembersite->UpdateTopicSelected($course_id,$week_no,$stud_id,$topic_selected))
	{
		echo 'Error logged. Cannot connect to server.';
	}
	
}
//gettin dem topics yall!
...
<div id="note" style="text-align: center;";>
		<h4><font color="red"> Caution:</font> Navigating away from this page after,during or before recording will count as your submission.</h4>
		</div>
...
Step 3: Once the 15 seconds reaches zero, it’s time to present<br>Good luck!
		...
		<div class="span4 offset1" style="" >
			...
			<div class="container" id="stopwatch" style="display: inline">
				<input class="btn btn-large btn-block btn-info" id="present" name="pButton" value="Present" type="button" onclick="buttontocd(); getQ('<?php echo $topic_selected;?>');  document.getElementById('present').disabled=true;"/>
			</div>
			<br>
			...
			<div class="container" id="playerbtndiv" style="display: inline">
				<input class="btn btn-large btn-block btn-success" id="playerbtn" name="pyButton" value="History" type="button" onclick="window.location='<?php echo "../student/history.php?course_id=$course_id";?>'"/>
			</div>
			<br>
			...
			<br>
			<p>Technical Issues? <a href="../contact.php">Contact support </a></p>
		</div>
	</div>
</div>
		...
		<script type="application/javascript">
			...
			function disablebtns() {
				...
				setTimeout(function() {document.getElementById('dashbtn').disabled=true}, 50);
			}

...
//if the status bar has week number ... else send us a report
session_start();
...
//initializing this variable is important as fail safe for recorder to switch back to default value if no time is set
$time_set=0;
$time_set= $fgmembersite->GetTime($week_no,$course_id);

$direction=('../Wowza/content/'.$fgmembersite->Userid()."-week-".$week_no.'-course-'.$course_id.'.flv');
if (file_exists($direction)) 
{
	$fgmembersite->LogOut();
	$fgmembersite->RedirectToURL("../index.php");
	exit;
	//send us a report....not yet implemented!
}
$fileName = $fgmembersite->Userid()."-week-".$week_no.'-course-'.$course_id;
$fgmembersite->UpdateSubmission($course_id,$week_no,$stud_id,$fileName);
...
//default:120
$config['maxRecordingTime']=$time_set;
...
echo"Some thing went wrong! please contact support";
}
?>